...
def upload_resume(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logging.info("Received resume upload request")

    try:
        content_type = req.headers.get("Content-Type", "")
        
        resume_id = str(uuid.uuid4())  # generate a unique ID for this resume
        ...
        if "application/json" in content_type:
            ...
            blob_path = f"{resume_id}/raw_text.txt"
            ...
            return func.HttpResponse(
                json.dumps({"resume_id": resume_id, "blob_url": blob_url}),
                status_code=200,
            ...
            blob_path = f"{resume_id}/{filename}"
            blob_client = blob_service.get_blob_client(container=container_name, blob=blob_path)
